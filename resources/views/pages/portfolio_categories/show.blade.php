...
@section('content')
    ...
    <div class='row justify-content-md-center mb-4'>
        <div class='col-md-2 wow animated slideInDown slow'>
            <a href="/portfolio" class='h5-responsive text-capitalize'>All</a>
        </div>
        @foreach($portfolio_categories as $portfolio_category)
            <div class='col-md-2 wow animated slideInDown slow'>
                <a href="/portfolio_category/{{$portfolio_category->id}}" class='@if($portfolio_category->id == $portfolioCategory->id) border-bottom border-primary font-weight-bold text-indigo @endif h5-responsive text-capitalize'>{{$portfolio_category->category}}</a>
            </div>
        @endforeach
    </div>
    ...
    <div class="row d-flex justify-content-center">
        @foreach ($portfolioCategory->portfolio as $portfolio)
            <!-- Grid column -->
